...
import com.sxwz.refreshlib.loadmore.GridViewHandler;
import com.sxwz.refreshlib.loadmore.ILoadMoreViewFactory;
import com.sxwz.refreshlib.loadmore.ListViewHandler;
import com.sxwz.refreshlib.loadmore.LoadMoreHandler;
import com.sxwz.refreshlib.loadmore.OnLoadMoreListener;
import com.sxwz.refreshlib.loadmore.OnScrollBottomListener;
import com.sxwz.refreshlib.loadmore.RecyclerViewHandler;
import com.sxwz.refreshlib.utils.PtrCLog;
 ...
 * description:
 *****************************************************/
public class BaseRefreshView extends ViewGroup {
    ...
    private UIHandlerHolder mUIHandlerHolder = UIHandlerHolder.create();
    private RefreshHandler mPtrHandler;
    // working parameters
    ...
    public BaseRefreshView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        mPtrIndicator = new PtrIndicator();

        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.BaseRefreshView, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(R.styleable.BaseRefreshView_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.BaseRefreshView_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.BaseRefreshView_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(R.styleable.BaseRefreshView_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(R.styleable.BaseRefreshView_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable.BaseRefreshView_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.BaseRefreshView_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.BaseRefreshView_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 2) {
            throw new IllegalStateException("BaseRefreshView only can host 2 elements");
            ...
            if (mContent == null || mHeaderView == null) {
                ...
                if (child1 instanceof UIHandler) {
                    mHeaderView = child1;
                    mContent = child2;
                } else if (child2 instanceof UIHandler) {
                    mHeaderView = child2;
                    mContent = child1;
                } else {
            ...
            errorView.setText("The content view in BaseRefreshView is empty. Do you forget to specify its id in xml layout file?");
            mContent = errorView;
            addView(mContent);
        }
        if (mHeaderView != null) {
    ...
    private void updatePos(int change) {
        ...
        if (mUIHandlerHolder.hasHandler()) {
            mUIHandlerHolder.onUIPositionChange(this, isUnderTouch, mStatus, mPtrIndicator);
        }
    ...
    public void setPtrHandler(RefreshHandler ptrHandler) {
        mPtrHandler = ptrHandler;
    }

    public void addPtrUIHandler(UIHandler ptrUIHandler) {
        UIHandlerHolder.addHandler(mUIHandlerHolder, ptrUIHandler);
    }

    @SuppressWarnings({"unused"})
    public void removePtrUIHandler(UIHandler ptrUIHandler) {
        mUIHandlerHolder = UIHandlerHolder.removeHandler(mUIHandlerHolder, ptrUIHandler);
    }
